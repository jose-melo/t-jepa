# This code was inspired by the TransformerEncoder implementation from PyTorch
# Source: https://pytorch.org/docs/stable/generated/torch.nn.TransformerEncoder.html
...
class TransformerEncoderLayer(Module):
    ...
    def forward(
        ...
        if self.norm_first:
            ...
            _debug_values(x[0].T, title="After self-attention residual sum")
            x = self.norm1(x)
            _debug_values(x[0].T, title="After norm1")
            _debug_values(
                ((x - self.norm1.bias) / self.norm1.weight)[0].T,
                title="Descaled x",
            )
            x = x + self._ff_block(x)
            _debug_values(x[0].T, title="After residual sum")
            x = self.norm2(x)
            _debug_values(x[0].T, title="After norm2")
    ...
    def _sa_block(
        ...
        _debug_values(x[0].T, "After self-attention")
        x = self.dropout1(x)
        _debug_values(x[0].T, "After dropout")
        return x
    ...
    def _ff_block(self, x: Tensor) -> Tensor:
        ...
        x = self.dropout2(x)
        _debug_values(x[0].T, "After feedforward")
        return x
